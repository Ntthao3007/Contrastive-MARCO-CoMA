...
def generate_negatives(model, input_ids, num_to_generate=6):
    negative_texts = random.sample(negative_samples, num_to_generate)
    negative_tokenized = tokenizer.batch_encode_plus(
        negative_texts, padding=True, truncation=True, return_tensors='pt'
    ).to(device)
    return negative_texts, negative_tokenized

def get_score(model, x, y, i, j):
    outputs = model(
        input_ids=x['input_ids'][i].reshape(1, -1), 
        attention_mask=x['attention_mask'][i].reshape(1, -1), 
        labels=y['input_ids'][j].reshape(1, -1)
    )
...
def compute_contrastive_loss(model, x, y):
    ...
    for i in range(batch_size):
        texts, negative_tokenized = generate_negatives(model, x['input_ids'][i].reshape(1, -1))
        target_score = get_score(model, x, y, i, i)
        denum = target_score
        for j in range(len(texts)):
            s = get_score(model, x, negative_tokenized, i, j)
            denum = denum + s
...
def main(args):
    global negative_samples  # Ensure negative_samples is accessible in generate_negatives
    ...
    if "jigsaw" in args.data_type:
        ...
        # Load negative samples with label 'hate' for contrastive learning
        negative_samples = df[df.label == "hate"].text.tolist()
